...
#gBGC_genomewide_stats <- read.csv(file = file.choose() , header = T, sep = ";", dec =".", stringsAsFactors = F)
leptidea_Bcor_M1_estimates_NG <- data.frame(Population = unique(factor(comb_gw_bs$Population, levels=c("Ire_juv","Kaz_juv", "Spa_rea", "Kaz_sin", "Swe_sin", "Spa_sin"))), Bcor_M1 =gBGC_genomewide_stats[gBGC_genomewide_stats$Filter=="noExons",]$Bcor_M1 )
...
ggplot(comb_gw_bs, aes(x=factor(comb_gw_bs$Population, levels=c("Ire_juv","Kaz_juv", "Spa_rea", "Kaz_sin", "Swe_sin", "Spa_sin")), y=Bcor_M1, fill = Population)) + geom_violin(draw_quantiles = 0.5) + scale_fill_manual(values = c(col[2], col[5], col[4],  col2[2], col[1], col[3])) + theme_classic() + 
  ...
  ylim(0,1) +
  geom_violin(data=comb_gw_bs, aes(x=factor(comb_gw_bs$Population, levels=c("Ire_juv","Kaz_juv", "Spa_rea", "Kaz_sin", "Swe_sin", "Spa_sin")), y=B_M1, fill=Population, alpha=Population), draw_quantiles = 0.5) +
  geom_violin(draw_quantiles = 0.5)+
  geom_point(data=leptidea_Bcor_M1_estimates_NG, shape=1, aes(x=Population, y=Bcor_M1))+
  scale_x_discrete(labels=c("Ire-juv","Kaz-juv", "Spa-rea", "Kaz-sin", "Swe-sin", "Spa-sin"))

#### Genomewide bs no genes no CpG ####
...
#### Genomewide bs - no Genes CpGonly ####
...
#Needs GC_centiles.results from the script GC_centiles.R
...
ggplot(gBGC_genomewide_stats[gBGC_genomewide_stats$Filter == "noExons",], aes(x=Chr_num_low, y=Bcor_M1, col=Population)) + geom_point(size=4) + geom_point(size=4, aes(x=Chr_num_high))+  
  ...
  annotation_custom(ggplotGrob(p3), xmin = 94, xmax = 129.9, 
                  ymin = 0.45, ymax = 0.908)
"6.04 x 5"
ggplot(gBGC_genomewide_stats[gBGC_genomewide_stats$Filter == "noExons",], aes(x=Chr_num_low, y=pi, col=Population)) + geom_point(size=3) + geom_point(size=3, aes(x=Chr_num_high)) + geom_point(size=3, aes(x=Chr_num_mean), shape=4) +  
  scale_color_manual(name="Population", values = c(col[2], col[5], col[4],  col2[2], col[1], col[3])) + 
  theme_classic() + 
  ylab("pi") +
  xlab("Diploid chromosome number") +
  geom_segment(aes(x=Chr_num_low, y=pi, xend=Chr_num_high, yend=pi), lty=3) +
...
#cor.test(pipic$contrasts, Bpic$contrasts)
#cor.test(chrpic$contrasts, Bpic$contrasts)
